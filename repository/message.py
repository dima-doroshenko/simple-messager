...
class Message(AbstractDTO):
    ...
    def __init__(self, chat: "Chat", message_obj: MessagesOrm):
        self.crud = chat.crud
    ...
    async def delete(self) -> None:
        stmt = delete(MessagesOrm).where(self.whereclause)
        await self.session.execute(stmt)

    async def edit(self, new_text: str) -> None:
        stmt = update(MessagesOrm).where(self.whereclause).values(text=new_text)
        await self.session.execute(stmt)
