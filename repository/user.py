...
class User(AbstractDTO):
    ...
    def login(self, credentials: HTTPBasicCredentials, response: Response):
    ...
    async def create_private_chat(self, data: CreatePrivateChat) -> int:

        if data.with_user == self.obj.id:
            raise AnswerException(
                status_code=status.HTTP_409_CONFLICT,
                msg="Can not create a private chat with yourself",
            )
        await self.crud.check_if_users_exist(data.with_user)
        await self.check_if_private_chat_exists(data.with_user)

        chat = PrivateChatsOrm(user_1=self.obj.id, user_2=data.with_user)
        self.session.add(chat)
        await self.session.flush()

        return chat.id

    async def create_group_chat(self, data: CreateGroupChat) -> int:
        with_users = data.with_users.copy()
        await self.crud.check_if_users_exist(with_users)

        chat = GroupChatsOrm(
            name=data.name, description=data.description, owner_id=self.obj.id
        )
        self.session.add(chat)
        await self.session.flush()

        chat_members = (
            with_users if self.obj.id in with_users else (self.obj.id, *with_users)
        )
        objects = [
            UserChatsOrm(user_id=user_id, chat_id=chat.id) for user_id in chat_members
        ]
    ...
    async def get_group_chat(
        ...
        if obj:
            ...
            if check_if_owner and (obj.owner_id != self.obj.id):
                raise AnswerException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    msg="You are not the owner of this chat",
                )
    ...
    async def get_private_chat(
        ...
        query = select(PrivateChatsOrm).where(
        ...
        if raise_exc:
            raise AnswerException(
                status_code=status.HTTP_404_NOT_FOUND,
                msg=f"Chat with user id {other_user_id} not found",
            )

    async def check_if_private_chat_exists(self, user_id: int, raise_exc: bool = True):
        ...
        if raise_exc and result:
            raise AnswerException(
                status_code=status.HTTP_409_CONFLICT,
                msg="Chat with this user has already created",
            )
    ...
    async def get_chat_ids_list(self) -> list[int]:
        ...
        query = select(UserChatsOrm).where(UserChatsOrm.user_id == self.obj.id)
        res = await self.session.execute(query)
        for obj in res.scalars().all():
